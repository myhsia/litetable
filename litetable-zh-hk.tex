...
\usepackage[linespread = 1.3, fontset = fandol]{ctex}
...
\author{%
  ...
  \texttt{<\href{mailto:myhsia@outlook.com}{myhsia@outlook.com}>}\thanks{%
    ...
    開發了讀取 \meta{left} \texttt{->} \meta{right} 型數據結構的接口，
    并为低版本 \hologo{TeX} Live 做兼容.
  }
}

\date{Released 2025-05-23\quad \texttt{v3.3H}}
...
\begin{function}{\weeklist}
  ...
  \begin{keyval}
    ...
    \item [\key{sep}] \val{string} 可設定工作日列表的分隔符
    （默認为空）.
  \end{keyval}
  \begin{verbatim}
